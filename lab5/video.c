...
void *(vg_init)(uint16_t mode)
{
    ...
    unsigned int vram_size = h_res * v_res * ceil(bits_per_pixel / (double)8); /* VRAM's size, but you can use the frame-buffer size, instead */
...
int vg_draw_pixel(uint16_t x, uint16_t y, uint32_t color)
{
    ...
    char *point = (char *)video_mem + (y * h_res + x) * bytesUsed;
